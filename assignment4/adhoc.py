        ...
        for sock in reading:
            try:
                ...
                if msg.startswith("DISCOVER:"):
                    ...
                    rtt = (time.time() - float(timestamp)) * 1000  # Convert to milliseconds
                    ...
                    if msg_id not in seen_messages:
                        ...
                        if nodes[-1] != target:
                            ...
                            logging.info(f"Received complete route request for {source} ---> {target} with nodes: {nodes}")
                            send_route_reply(msg_id, source, target, nodes, nodes[-2])  # Send reply to the previous node
                    ...
                    idx = nodes.index(IP)
                    if idx == 0:
                        logging.info(f"Received complete route reply for {source} ---> {target} with nodes: {nodes}")
                        assert nodes.pop(0) == IP, "First node in RREP must be the local IP"
